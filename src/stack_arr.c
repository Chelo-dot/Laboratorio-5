...
void sa_init(StackArr *s, int cap){
    ...
    DinaArray arreglo = crear_arreglo(cap);
    s->arr = arreglo;
...
void sa_push(StackArr *s, int valor){
    ...
    agregar_elemento(&s->arr, s->arr.size-s->arr.capacidad, valor);

}

int sa_pop(StackArr *s){

    int valor_arriba = *(s->arr.data + (s->arr.size-s->arr.capacidad - 1));
    eliminar_elemento(&s->arr, s->arr.size-s->arr.capacidad - 1);
    return valor_arriba;
